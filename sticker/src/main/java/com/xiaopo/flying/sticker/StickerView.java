...
public class StickerView extends FrameLayout {
    ...
    switch (currentMode) {
      ...
      case ActionMode.DRAG:
        if (handlingSticker != null) {
          ...
          Log.d(TAG, "event.getX() - downX = "+event.getX()+
                  "\n event.getY() - downY = "+event.getY());
  ...
  //Kanika : June 18, 2018
  public StickerView addStickerAt(final Bitmap bitmap, @NonNull final Sticker sticker,
                                  final int positionX, final int positionY) {
      if (ViewCompat.isLaidOut(this)) {
          addStickerImmediatelyAt(bitmap, sticker, positionX, positionY);
      } else {
          post(new Runnable() {
              @Override public void run() {
                  addStickerImmediatelyAt(bitmap, sticker,positionX, positionY);
    ...
    protected void addStickerImmediatelyAt(Bitmap bitmap, @NonNull Sticker sticker, int positionX, int positionY) {
        setStickerPositionAt(bitmap, sticker, positionX, positionY);
//
//
//        float scaleFactor, widthScaleFactor, heightScaleFactor;
//
//        widthScaleFactor = (float) getWidth() / sticker.getDrawable().getIntrinsicWidth();
//        heightScaleFactor = (float) getHeight() / sticker.getDrawable().getIntrinsicHeight();
////    widthScaleFactor = (float) 150;
////    heightScaleFactor = (float) 150;
//        scaleFactor = widthScaleFactor > heightScaleFactor ? heightScaleFactor : widthScaleFactor;
////*************************************************************************************************************
////    sticker.getMatrix()
////        .postScale(scaleFactor / 2, scaleFactor / 2, getWidth() / 2, getHeight() / 2);
//
//        sticker.getMatrix()
//                .postScale(0.03f, 0.03f, positionX, positionY);
//
//        handlingSticker = sticker;
//        stickers.add(sticker);
//        if (onStickerOperationListener != null) {
...
//        sticker.getMatrix().postTranslate(positionX, positionY);// your location
        handlingSticker = sticker;
        stickers.add(sticker);
        if (onStickerOperationListener != null) {
            onStickerOperationListener.onStickerAdded(sticker);
...
//      if (handlingSticker != null) {
//        moveMatrix.set(downMatrix);
//        moveMatrix.postTranslate(positionX - downX, positionY);
//        handlingSticker.setMatrix(moveMatrix);
//        if (constrained) {
//          constrainSticker(handlingSticker);
//        }
...
//        invalidate();
    ...
    protected void setStickerPositionAt(@NonNull Sticker sticker, int positionX, int positionY) {
        float width = getWidth();
        float height = getHeight();
//        float offsetX = width - sticker.getWidth();
//        float offsetY = height - sticker.getHeight();

      float offsetX = width - positionX;
      float offsetY = height - positionX;
        if ((positionY & Sticker.Position.TOP) > 0) {
            offsetY /= 4f;
        } else if ((positionY & Sticker.Position.BOTTOM) > 0) {
            offsetY *= 3f / 4f;
        } else {
            offsetY /= 2f;
        }
        if ((positionX & Sticker.Position.LEFT) > 0) {
            offsetX /= 4f;
        } else if ((positionX & Sticker.Position.RIGHT) > 0) {
            offsetX *= 3f / 4f;
        } else {
            offsetX /= 2f;
        }
        sticker.getMatrix().postTranslate(positionX, positionY);
    }

  protected void  setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
    float width = getWidth(); //view width
    float height = getHeight(); //view height;
    float offsetX = width - sticker.getWidth();     //viewWidth - imageWidth
    float offsetY = height - sticker.getHeight();   //viewHeight - imageHeight
  ...
  protected void  setStickerPositionAt(Bitmap bitmap, @NonNull Sticker sticker, int positionX, int positionY) {
    float width = bitmap.getWidth(); //bitmap width
    float height = bitmap.getHeight(); //bitmap height;
    float offsetX = width - sticker.getWidth();     //viewWidth - imageWidth
    float offsetY = height - sticker.getHeight();   //viewHeight - imageHeight
    if ((positionY & Sticker.Position.TOP) > 0) {
      offsetY /= 4f;
